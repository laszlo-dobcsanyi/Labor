...
namespace Labor
{
    public struct Vizsgálat
    {
        public struct Azonosító
        {
            ...
            public Azonosító(string _termékkód, string _hordószám, string _sarzs, int? _sorszám)
            {
                sorszám = _sorszám;
        ...
        public struct Adatok1
        {
            ...
            public Adatok1(string _terméknév, string _szita_átmérő, int _hőkezelés, double _nettó_töltet, string _gyártási_év, string _műszak_jele, string _töltőgép, string _hordótípus, string _megrendelő, string _szárm_ország, string _gyümölcsfajta)
            {
    ...
    public sealed class Panel_Vizsgálatok : Control
    {
        ...
        // TODO Nem kell az egész vizsgálatot tokenelni!
        private List<DataToken<Vizsgálat>> vizsgálat_tokenek = new List<DataToken<Vizsgálat>>();
        ...
        private DataTable CreateSource()
        {
            ...
            data.Columns.Add(new DataColumn("Nettó töltet", System.Type.GetType("System.Int32")));
            ...
            List<Vizsgálat> vizsgálatok = Program.database.Vizsgálatok();

            foreach (Vizsgálat item in vizsgálatok)
            {
                DataRow row = data.NewRow();
                row[0] = item.azonosító.termékkód;
                row[1] = item.azonosító.sarzs;
                row[2] = item.azonosító.hordószám;
                row[3] = item.adatok1.hordótípus;
                row[4] = item.adatok1.nettó_töltet;
                row[5] = item.adatok1.szita_átmérő;
                row[6] = item.adatok1.megrendelő;
                data.Rows.Add(row);
            }
            return data;
        }
        ...
        private void módosítás_Click(object sender, DataGridViewCellEventArgs e)
        {
            ...
            Vizsgálat.Azonosító azonosító = new Vizsgálat.Azonosító((string)table.SelectedRows[0].Cells[0].Value, (string)table.SelectedRows[0].Cells[2].Value,
                (string)table.SelectedRows[0].Cells[1].Value, (int)table.SelectedRows[0].Cells[7].Value);

            foreach (DataToken<Vizsgálat> token in vizsgálat_tokenek)
            {
                if (token.data.azonosító.Equals(azonosító))
                {
                    Vizsgálati_Lap vizsgálati_lap = new Vizsgálati_Lap(token.data);
                    vizsgálati_lap.ShowDialog();
                    break;
                }
            }
        ...
        public sealed class Vizsgálati_Lap : Form
        {
            ...
            private void InitializeContent()
            {
                ...
                Label műszak_jele = Program.mainform.createlabel("Műszak jele:", sarzs.Location.X, szita_átmérő.Location.Y,this);
                Label töltőgép_száma = Program.mainform.createlabel("Töltőgép száma:", sarzs.Location.X + oszlop, szita_átmérő.Location.Y,this);
                ...
                Label gyümölcsfajta = Program.mainform.createlabel("Gyümölcsfajta", hordótípus.Location.X, megrendelő.Location.Y,this);
                ...
                combo_gyümölcsfajta = Program.mainform.createcombobox(gyümölcsfajta.Location.X + gyümölcsfajta.Width + köz, gyümölcsfajta.Location.Y, méret[4],this); combo_hordótípus.Location = new Point(combo_gyümölcsfajta.Location.X, combo_hordótípus.Location.Y);
                ...
                #region Events
                box_termékkód.KeyDown += box_termékkód_KeyDown;
                box_hordószám.Enter += box_hordószám_Enter;
                box_hordószám.Leave += box_hordószám_Leave;
                ...
                List<string> gyümölcsfajták = Program.database.Gyümölcsfajták();
            ...
            private void InitializeData()
            {
                foreach (Control item in this.Controls)
                {
                    if (item is ComboBox)
                    {
                        ComboBox temp = item as ComboBox;
                        temp.Items.Add("");
                    }
                }
            }
            ...
            private void InitializeData(Vizsgálat _vizsgálat)
            {
                ...
                box_szita_átmérő.Text = _vizsgálat.adatok1.szita_átmérő;
                box_hordószám.Text = _vizsgálat.azonosító.hordószám;
                ...
                box_nettó_töltet.Text = _vizsgálat.adatok1.nettó_töltet.ToString();
                ...
                combo_hordótípus.SelectedItem = _vizsgálat.adatok1.hordótípus;
                combo_megrendelő.SelectedItem = _vizsgálat.adatok1.megrendelő;
            }

            #region EventHandlers
            private void box_termékkód_KeyDown(object sender, KeyEventArgs e)
            {
                if (e.KeyCode == Keys.Enter)
                {
                    if (box_termékkód.Text.Length != 3) return;
                    List<string> temp = Program.database.Vizsgálat_Termékkódok(box_termékkód.Text);
            ...
            private void box_hordószám_Enter(object sender, EventArgs e)
            {
                // TODO visszatérés ellenőrzés!
                box_terméknév.Text = Program.database.Vizsgálat_Terméknév(box_termékkód.Text);
            }
            private void box_hordószám_Leave(object sender, EventArgs e)
            {
                string prodid = "12" + box_termékkód.Text + gyártási_év[gyártási_év.Length - 1] + "_0" + gyártási_év[gyártási_év.Length - 1] + box_hordószám.Text;
            ...
            private void combo_hordótípus_Leave(object sender, EventArgs e)
            {
            ...
            void rendben_Click(object sender, System.EventArgs e)
            {
                ...
                foreach (Control item in this.Controls)
                {
                    if (item is TextBox)
                    {
                        ...
                        if (temp.MaxLength < temp.Text.Length)
                        {
                            item.Text = item.Text.Remove(temp.MaxLength - 1);
                        }
                    }
                    if (item is ComboBox)
                    {
